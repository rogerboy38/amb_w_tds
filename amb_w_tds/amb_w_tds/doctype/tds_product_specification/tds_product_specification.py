...
class TDSProductSpecification(Document):
    ...
    def validate_version_control(self):
        ...
        if not self.tds_version:
            ...
            latest = frappe.db.get_value(
                ...
                fieldname='tds_version',
                order_by='tds_version desc'
            )
            self.tds_version = (latest or 0) + 1
        ...
        if existing:
            frappe.throw(_(f"Version {self.tds_version} already exists for {self.product_item}"))
    ...
    def revert_version_status(self):
        ...
        if self.tds_version > 1:
            previous_version = frappe.db.get_value(
                'TDS Product Specification',
                filters={
                    'product_item': self.product_item,
                    'tds_version': self.tds_version - 1,
                    'docstatus': 1
                },
                fieldname='name'
            )
...
def get_latest_tds_version(product_item):
    ...
    tds = frappe.db.get_value(
        ...
        fieldname=['name', 'tds_version', 'approval_date'],
        order_by='tds_version desc',
        as_dict=True
    )
...
def get_tds_history(product_item):
    ...
    return frappe.get_all(
        ...
        fields=['name', 'tds_version', 'approval_date', 'approved_by', 'docstatus', 'is_archived'],
        order_by='tds_version desc'
    )
